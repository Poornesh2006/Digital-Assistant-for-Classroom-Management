...
from classroom_app.config import BASE_DIR, CHARTS_FOLDER
from classroom_app.services.data import _safe_slug
...
def generate_student_charts(student_name: str, semester_totals: list[int], semester_attendance: list[int]) -> dict[str, str]:
    ...
    return {
        "performance_chart": f"charts/{perf_file}",
        "attendance_chart": f"charts/{att_file}",
        "contribution_chart": f"charts/{pie_file}",
    }
